...
def ppo(env, net, episodes, env_version, net_version, plot_episode, alpha = 1e-4, gamma = 0.99, beta = 0.02, clip = 0.2, landa = 0.95):
    ...
    for episode in tqdm(range(1, episodes + 1)):
        ...
        while not done:
            ...
            I = I * gamma
            D = D * landa
            step_data.append([state_c, action.clone(), reward.clone(), policy.clone().detach(), value.clone().detach(), value_next_state.clone().detach(), I, D])
            state = next_state
            ep_return += reward
            step = step + 1 
        ...
        if episode in plot_episode:
            plot_prog(env, episode, net, env_version, net_version ,"figures", "ppo" )
