...
env = FireGrid_V8(20, burn_value=10, n_sims = 1, n_sims_final = 50 )
...
for i in range(100):
    ...
    plt.savefig(f"figures/{env_version}/{net_version}/ppo/pre_train/probs_before_training_"+ str(i) +".png")
    plt.show()
...
stats = ppo(env, net, episodes, env_version, net_version, plot_episode, beta=0.1)

# Guardamos los parametros de la red
if save:
    path = f"./weights/{env_version}/{net_version}/ppo.pth"
    torch.save(net.state_dict(), path)
############################# Plots #################################################################
plot_moving_av(stats["Returns"], episodes, env_version, net_version, "ppo", window = 10)


figure3 = plt.figure()
plt.clf()
plt.plot([i for i in range(episodes + 1) if i%update == 0 and i != 0], stats["Actor Loss"])
plt.xlabel("Episode") 
plt.ylabel("Loss") 
plt.title(f"Loss for {episodes} episodes")
plt.savefig(f"figures/{env_version}/{net_version}/ppo/actor_loss.png")
plt.show() 

figure4 = plt.figure()
plt.clf()
plt.plot([i for i in range(episodes + 1) if i%update == 0 and i != 0], stats["Critic Loss"])
plt.xlabel("Episode") 
plt.ylabel("Loss") 
plt.title(f"Loss for {episodes} episodes")
plt.savefig(f"figures/{env_version}/{net_version}/ppo/critic_loss.png")
plt.show() 
