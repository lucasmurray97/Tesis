...
from algorithms.ppo_coupled_v3 import ppo
...
for i in range((env.size//2)**2):
    ...
    plt.savefig(f"{path}/{env_version}/{instance}/{net_version}/ppo_v3/pre_train/probs_before_training_"+ str(i) +".png")
    plt.show()
...
############################# Plots #################################################################
plot_moving_av(stats["Returns"], episodes*n_envs, env_version, net_version, "ppo_v3", window = window, instance = instance, test = test)


figure3 = plt.figure()
plt.clf()
plt.plot([i for i in range(episodes)], stats["Actor Loss"])
plt.xlabel("Episode") 
plt.ylabel("Loss") 
plt.title(f"Loss for {episodes} episodes")
plt.savefig(f"{path}/{env_version}/{instance}/{net_version}/ppo_v3/actor_loss.png")
plt.show() 

figure4 = plt.figure()
plt.clf()
plt.plot([i for i in range(episodes)], stats["Critic Loss"])
plt.xlabel("Episode") 
plt.ylabel("Loss") 
plt.title(f"Loss for {episodes} episodes")
plt.savefig(f"{path}/{env_version}/{instance}/{net_version}/ppo_v3/critic_loss.png")
plt.show() 
