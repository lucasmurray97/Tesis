...
from batched_reinforce_baseline import batched_reinforce_baseline
...
for i in range(100):
    ...
    plt.savefig(f"figures/{env_version}/{net_version}/batched_reinforce_baseline/pre_train/probs_before_training_"+ str(i) +".png")
    plt.show()
...
stats = batched_reinforce_baseline(env, net, episodes, env_version, net_version, plot_episode, beta=0.1)

# Guardamos los parametros de la red
if save:
    path = f"./weights/{env_version}/{net_version}/batched_reinforce_baseline.pth"
    torch.save(net.state_dict(), path)

plot_moving_av(stats["Returns"], episodes, env_version, net_version, "batched_reinforce_baseline", window = 10)

figure3 = plt.figure()
plt.clf()
plt.plot([i for i in range(episodes + 1) if i%batch == 0 and i != 0], stats["Actor Loss"])
plt.xlabel("Episode") 
plt.ylabel("Loss") 
plt.title(f"Loss for {episodes} episodes")
plt.savefig(f"figures/{env_version}/{net_version}/batched_reinforce_baseline/actor_loss.png")
plt.show() 

figure4 = plt.figure()
plt.clf()
plt.plot([i for i in range(episodes + 1) if i%batch == 0 and i != 0], stats["Critic Loss"])
plt.xlabel("Episode") 
plt.ylabel("Loss") 
plt.title(f"Loss for {episodes} episodes")
plt.savefig(f"figures/{env_version}/{net_version}/batched_reinforce_baseline/critic_loss.png")
plt.show() 
