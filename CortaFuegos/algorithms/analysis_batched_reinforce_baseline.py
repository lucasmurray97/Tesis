...
for i in range(100):
    ...
    plt.savefig(f"figures/{version}/batched_reinforce_baseline/pre_train/probs_before_training_"+ str(i) +".png")
    plt.show()
...
stats = batched_reinforce_baseline(env, net, episodes, version, plot_episode, beta=0.1)

# Guardamos los parametros de la red
if save:
    path = "./weights/batched_reinforce_baseline.pth"
...
plt.savefig(f"figures/{version}/batched_reinforce_baseline/returns.png")
plt.show() 
...
plt.savefig(f"figures/{version}/batched_reinforce_baseline/actor_loss.png")
plt.show() 
...
plt.ylabel("Loss") 
plt.title(f"Loss for {episodes} episodes")
plt.savefig(f"figures/{version}/batched_reinforce_baseline/critic_loss.png")
plt.show() 
