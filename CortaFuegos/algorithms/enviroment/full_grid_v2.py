...
class Full_Grid_V2(Abstract_Full_Grid):
    ...
    def generate_matrix(self):
        ...
        adj_tensor = torch.from_numpy(adj.astype(int)).to(self.device)
        return adj_tensor
    ...
    def reset(self):
        ...
        stacked_state = torch.stack((padded_state, self.adjacency_matrix)).to(self.device)
    ...
    def step(self, action):
        ...
        state = torch.stack((self.pad_state(s), self.adjacency_matrix)).to(self.device)
        done = False
        if self._step == self.get_episode_len()-1:
            ...
            self._step = 0
            return state, torch.Tensor([reward]).reshape((1, 1)).to(self.device), done
        self._step += 1
        return state, torch.zeros(1).to(self.device), done
