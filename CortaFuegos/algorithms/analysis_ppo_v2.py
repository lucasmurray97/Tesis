...
for i in range(100):
    ...
    plt.savefig(f"{path}/{env_version}/{instance}/{net_version}/ppo_v2/pre_train/probs_before_training_"+ str(i) +".png")
    plt.show()
...
stats = ppo_v2(env, net, episodes, env_version, net_version, plot_episode, alpha = alpha, gamma = gamma, landa = landa, beta = beta, instance = instance, test = test)

# Guardamos los parametros de la red
if save:
    path_ = f"./weights/{env_version}/{net_version}/ppo_v2.pth"
    torch.save(net.state_dict(), path_)
############################# Plots #################################################################

plot_moving_av(stats["Returns"], episodes*n_envs, env_version, net_version, "ppo_v2", window = window, instance = instance, test = test)
if size == 2:
    updates = 1
else:
    updates = 2
figure3 = plt.figure()
plt.clf()
plt.plot(np.arange(episodes*updates), stats["Actor Loss"])
plt.xlabel("Episode") 
plt.ylabel("Loss") 
plt.title(f"Loss for {episodes} episodes")
plt.savefig(f"{path}/{env_version}/{instance}/{net_version}/ppo_v2/actor_loss.png")
plt.show() 

figure4 = plt.figure()
plt.clf()
plt.plot(np.arange(episodes*updates), stats["Critic Loss"])
plt.xlabel("Episode") 
plt.ylabel("Loss") 
plt.title(f"Loss for {episodes} episodes")
plt.savefig(f"{path}/{env_version}/{instance}/{net_version}/ppo_v2/critic_loss.png")
plt.show() 
