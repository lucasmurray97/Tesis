...
from ppo_coupled_v2 import ppo_v2

# We create the enviroment
env = FireGrid_V7(20, burn_value=10, n_sims=1, n_sims_final = 50)
...
for i in range(100):
    ...
    plt.savefig(f"figures/{env_version}/{net_version}/ppo_v2/pre_train/probs_before_training_"+ str(i) +".png")
    plt.show()
...
stats = ppo_v2(env, net, episodes, env_version, net_version, plot_episode, beta=0.1)

# Guardamos los parametros de la red
if save:
    path = f"./weights/{env_version}/{net_version}/ppo_v2.pth"
    torch.save(net.state_dict(), path)
############################# Plots #################################################################

plot_moving_av(stats["Returns"], episodes, env_version, net_version, "ppo_v2", window = 10)
...
plt.savefig(f"figures/{env_version}/{net_version}/ppo_v2/actor_loss.png")
plt.show() 
...
plt.ylabel("Loss") 
plt.title(f"Loss for {episodes} episodes")
plt.savefig(f"figures/{env_version}/{net_version}/ppo_v2/critic_loss.png")
plt.show() 
