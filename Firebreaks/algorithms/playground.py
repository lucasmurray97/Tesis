...
env_shape = (1, 20, 20)
memory = ReplayMemory(env_shape, max_mem=15, batch_size=10, demonstrate=True, n_dem=10, prioritized=False, env="FG", version=1, size=env_shape[1],n_envs=16, gamma = 1., landa = 1.)
print(memory.buffer.action_memory)
step = 0
state = env.reset()
action = env.random_action()
next_state, reward, done = env.step(action)
memory.buffer.store_transition(state, action.unsqueeze(0), reward, next_state, done, 0.99, 0.99)
action = env.random_action()
print(memory.buffer.action_memory)
print(memory.buffer.dem_pivot)
memory.buffer.store_transition(state, action.unsqueeze(0), reward, next_state, done, 0.99, 0.99)
action = env.random_action()
print(memory.buffer.action_memory)
print(memory.buffer.dem_pivot)
