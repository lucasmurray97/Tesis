...
class CNN_BIG_V2(torch.nn.Module):
  def __init__(self, grid__size = 20, input_size = 2, output_size = 400, value = True, forbidden = []):
    ...
    self.conv2 = nn.Conv2d(in_channels=16, out_channels=16, kernel_size=(2,2), stride=2, padding = 0, bias = True)
    self.max_p1 = nn.MaxPool2d(2, stride=2)
    self.conv3 = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=(2,2), stride=2, padding = 0, bias = True)
    self.conv4 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=(2,2), stride=2, padding = 0, bias = True)
    ...
    if self.grid_size == 6:
      self.linear1 = nn.Linear(32, 48)
       ...
       self.linear1 = nn.Linear(4608, 48)
    ...
    if self.grid_size == 6:
      self.linear_1 = nn.Linear(32, 48)
       ...
       self.linear_1 = nn.Linear(4608, 48)
