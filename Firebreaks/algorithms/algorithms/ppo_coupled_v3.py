...
import json
def ppo(env, net, episodes, env_version, net_version, plot_episode, alpha = 1e-5, gamma = 0.99, beta = 0.02, clip = 0.2, landa = 0.95, n_envs = 8, epochs = 10, batches = 8, batch_size = 2, instance = "sub20x20", test = False, window = 10):
    ...
    for episode in tqdm(range(1, episodes + 1)):
        ...
        data = DataLoader([[state_t[i], action_t[i], reward_t[i], policy_t[i], entropy_t[i], mask_t[i] , value_t[i], value_next_state_t[i], discounts[i], landas[i]] for i in range(n_envs)], 1, shuffle = True)
        actor_loss_acum = 0
        critic_loss_acum = 0
        n = 0
        for state_t, action_t, reward_t, policy_t, entropy_t, mask_t, value_t, value_next_state_t, discounts, landas in data:
            ...
            for e in range(epochs):
                ...
                actor_loss = - (clip_loss + torch.sum(beta*entropy))
    ...
    plot_moving_av(env.envs[0], stats["Returns"], episodes*n_envs, env_version, net_version, "ppo_v3", window = window, instance = instance, test = test, params = params)
    plot_loss(env.envs[0], stats["Loss"], episodes, env_version, instance, net_version, "ppo_v3", test)
    params_dir = f"episodes={episodes*n_envs}_"
    for key in params.keys():
            params_dir += key + "=" + str(params[key]) + "_"
    with open(f"data/{env.envs[0].name}/{instance}/{env_version}/{net_version}/ppo_v3/stats_{params_dir}.json", "w+") as write_file:
