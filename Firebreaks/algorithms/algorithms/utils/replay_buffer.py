...
class ReplayMemoryBaseline:
    ...
    def store_transition(self, state, action, reward, state_, done, gamma, landa, q, target):
        for i in range(state.shape[0]):
            self.store_transition_indiv(state[i].cpu(), action[i].cpu(), reward[i].cpu(), state_[i].cpu(), done, gamma, landa, q, target)
    
    def store_transition_indiv(self, state, action, reward, state_, done, gamma, landa, q, target):
    ...
    def sample_memory(self):
        ...
        batch = np.random.choice(max_mem, 10,
                                 replace=False)
...
class PrioritizedReplayMemory:
    ...
    def store_transition(self, state, action, reward, state_, done, gamma, landa, q, target):
        for i in range(state.shape[0]):
            self.store_transition_indiv(state[i].cpu(), action[i].cpu(), reward[i].cpu(), state_[i].cpu(), done, gamma, landa, q[i].cpu(), target[i].cpu())
    
    def store_transition_indiv(self, state, action, reward, state_,done, gamma, landa, q, target, offset = 0.1):
        error = target - q.gather(0, action.type(torch.int64))
        index = (self.mem_cntr ) % (self.mem_size - self.dem_pivot) 
        self.state_memory[index + self.dem_pivot] = state
        self.action_memory[index + self.dem_pivot] = action
        self.reward_memory[index + self.dem_pivot] = reward
        ...
        self.landas_memory[index + self.dem_pivot] = landa
        self.dones_memory[index + self.dem_pivot] = done
        self.temporal_errors[index + self.dem_pivot] = abs(error.item()) + offset
        self.mem_cntr += 1
