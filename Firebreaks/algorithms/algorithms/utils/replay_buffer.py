...
class ReplayMemory:
    def __init__(self, input_dims, max_mem, batch_size, demonstrate = False, n_dem = None, combined=False, temporal = False, prioritized = False, env = "FG", version = 1, size = 20, n_envs = 16, gamma = 1., landa = 1.):
        ...
        if prioritized:
            ...
            self.buffer = ReplayMemoryBaseline(input_dims=input_dims, max_mem=max_mem, batch_size=batch_size, combined=combined, n_envs=n_envs)
        if self.demonstrate:
...
class ReplayMemoryBaseline:
    ...
    def store_dem(self, state, action, reward, state_, step, done, gamma, landa):
        for i in range(state.shape[0]):
            self.store_transition_indiv(state[i].cpu(), action[i].cpu(), reward[i].cpu(), state_[i].cpu(), step, done, gamma, landa)
        if done:
            self.mem_cntr += 1

    def store_dem_indiv(self, state, action, reward, state_, step, done, gamma, landa):
        ...
